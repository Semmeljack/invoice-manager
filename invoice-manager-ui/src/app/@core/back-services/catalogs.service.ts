...
export class CatalogsService {
    ...
    public getAllClavesProductoServicio(
        page: number,
        size: number
    ): Promise<any> {
        ...
        return this.httpClient
            .get('../api/catalogs/producto-servicios', { params: pageParams })
            .toPromise();
    }

    public getZipCodeInfo(zipCode: String): Promise<any> {
        return this.httpClient
            .get(`/api/catalogs/codigo-postal/${zipCode}`)
            .toPromise();
    }
    ...
    public getAllRegimenFiscal(): Promise<any> {
        return new Promise((resolve) => {
            if (this.regimenFiscal !== undefined) {
                ...
                this.httpClient
                    .get('/api/catalogs/regimen-fiscal')
                    .subscribe((cat) => {
    ...
    public getTiposReferencia(formapago: string): Promise<any> {
        return new Promise((resolve) => {
            if (formapago === 'EFECTIVO') {
                return resolve([
                    new Catalogo('DIRECCION', 'Direcccion de entrega'),
                ]);
            }
            if (formapago === 'CHEQUE') {
                return resolve([
            ...
            if (formapago === 'TRANSFERENCIA') {
                return resolve([
                    new Catalogo('CLABE', 'CLABE interbacaria'),
            ...
            if (formapago === 'FACTURA') {
                return resolve([
                    new Catalogo('FOLIO', 'Folio factura a pagar'),
                ]);
            }
            if (formapago === 'PAGO_MULTIPLE') {
                return resolve([
                    new Catalogo('ARCHIVO', 'Archivo con detalles de pago'),
                ]);
            }
            return resolve([]);
        });
