  ...
  public RoleDto getRoleByName(String name) throws InvoiceManagerException {
    ...
    if (result.isPresent()) {
      ...
      throw new InvoiceManagerException(
          "Role not found",
          String.format("Role with the name %s not found", name),
    ...
    User entity =
        userRepository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, String.format("user no existe %d", userId)));
    Role rol =
        repository
            .findByUserIdAndId(entity.getId(), id)
            .orElseThrow(
                () ->
                    new ResponseStatusException(
  ...
  public RoleDto insertNewRole(Integer userId, RoleDto roleDto) {
    ...
    if (rol.isPresent()) {
      throw new ResponseStatusException(
          ...
          String.format(
              "Ya existe el rol con el nombre %s para el user %d", roleDto.getRole(), userId));
    } else {
